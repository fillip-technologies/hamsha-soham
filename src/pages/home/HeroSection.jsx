...
export const HeroSection = () => {
  ...
  return (
    <section
      ref={heroRef}
      className="relative overflow-hidden pt-6 pb-10 sm:pt-20 sm:pb-28 bg-[#F8FAFD] text-slate-900 select-none border-b border-slate-200/80"
    >
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-8 lg:px-12 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-12 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-6 space-y-4 sm:space-y-6 text-left">
            ...
            {/* Master Headline */}
            <h1 className="text-[1.6rem] xs:text-3xl sm:text-4xl lg:text-[42px] xl:text-[48px] font-black tracking-tight leading-[1.15]">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 pt-1 max-w-xl">
              <div className="text-left space-y-0.5">
                <div className="text-xl sm:text-3xl font-black text-[#0B132B] tracking-tight">
                  <AnimatedCounter end={50} duration={2000} suffix="+" />
                </div>
                <div className="text-[11px] sm:text-xs text-slate-500 font-medium leading-tight">Multispeciality Hospitals</div>
              </div>
              <div className="text-left space-y-0.5">
                <div className="text-xl sm:text-3xl font-black text-[#FF4D27] tracking-tight">
                  <AnimatedCounter end={100} duration={1800} suffix="%" />
                </div>
                <div className="text-[11px] sm:text-xs text-slate-500 font-medium leading-tight">NABH & NABL Compliant</div>
              </div>
              <div className="text-left space-y-0.5">
                <div className="text-xl sm:text-3xl font-black text-[#0B132B] tracking-tight">
                  <AnimatedCounter end={99.99} decimals={2} duration={2200} suffix="%" />
                </div>
                <div className="text-[11px] sm:text-xs text-slate-500 font-medium leading-tight">Operational Uptime</div>
              </div>
              <div className="text-left space-y-0.5">
                <div className="text-xl sm:text-3xl font-black text-[#FF4D27] tracking-tight">24 / 7</div>
                <div className="text-[11px] sm:text-xs text-slate-500 font-medium leading-tight">Round-the-clock Support</div>
              </div>
            </div>

            {/* Action CTAs */}
            <div className="flex flex-col xs:flex-row flex-wrap items-start xs:items-center gap-3 sm:gap-4 pt-1">
              <a
                href="https://apps.hamsasoham.com/portal/index.xhtml"
                target="_blank"
                rel="noopener noreferrer"
                className="w-full xs:w-auto px-6 py-3 rounded-full bg-gradient-to-r from-[#FF4D27] to-[#FF6B4A] hover:from-[#E03A14] hover:to-[#FF4D27] text-white font-bold text-sm sm:text-base flex items-center justify-center gap-2.5 shadow-lg shadow-[#FF4D27]/25 hover:shadow-xl hover:shadow-[#FF4D27]/35 hover:-translate-y-0.5 transition-all cursor-pointer"
              >
              ...
              <a
                ...
                className="w-full xs:w-auto px-5 py-3 rounded-full bg-white hover:bg-slate-50 border border-slate-300 text-slate-800 font-semibold text-sm sm:text-base flex items-center justify-center gap-2 shadow-xs hover:border-orange-300 transition-all cursor-pointer"
              >
          ...
          <div className="lg:col-span-6 relative flex flex-col items-center justify-center group w-full">
            ...
            <div className="hidden sm:block w-16 sm:w-24 h-12 sm:h-16 mx-auto bg-gradient-to-b from-slate-200 via-slate-100 to-slate-300 border-x border-slate-300 -mt-1 [clip-path:polygon(20%_0%,80%_0%,100%_100%,0%_100%)] shadow-inner transition-transform duration-500 group-hover:-translate-y-0.5" />

            {/* Monitor Flat Horizontal Base */}
            <div className="hidden sm:block w-48 sm:w-68 h-3 sm:h-4 mx-auto bg-gradient-to-b from-white to-slate-100 border border-slate-300/90 rounded-full shadow-lg -mt-0.5 transition-transform duration-500" />
